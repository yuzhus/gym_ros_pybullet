...
if __name__ == '__main__':
    ...
    rospy.init_node('train_xarm_gym', anonymous=True)
    ...
    model.learn(total_timesteps, callback=callback, tb_log_name="first_run")

    plot_results([outdir], total_timesteps, results_plotter.X_TIMESTEPS, "Training XarmReachEnv with PPO")
    plt.savefig(f"{outdir}/reward_train.png", bbox_inches='tight')
    # plt.show()
    # plt.close()

    model.save(f"{outdir}/Trained_XarmReach")
    # model.save_replay_buffer(f"{outdir}/replay_buffer_v4")
